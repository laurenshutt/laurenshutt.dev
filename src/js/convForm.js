...
ConvState.prototype.printQuestion = function(){
    ...
    if (this.current.input.hasOwnProperty("showField")){
      console.log("showing");
      $("#userInput, #convForm > button").show();
      $("#convForm").css("box-shadow","0 0 5px 5px rgb(222 222 222 / 40%)");
    }

    function readySet(words){
    var wpm = 750;
        var time = parseFloat((words * 60)/wpm).toFixed(2);
    ...
    }.bind(this), readySet(question.toString().split(" ").length) * 1000)
};
